...
export class TitleScene extends Phaser.Scene {
    constructor() {
        ...
        this.menuOpts = ['start', 'highScores', 'howToPlay', 'settings', 'credits'];
        // Animation state
        this.animTime = 0;
        this.phase = 'walk';
        this.uncleX = 680;
    ...
    update(_time, delta) {
        ...
        // Music volume ramp
        if (t < 2000)
            this.audio.setBGMusicTargetVol(0.015);
        else if (t < 3000)
            this.audio.setBGMusicTargetVol(0.025);
        else if (t < 4000)
            this.audio.setBGMusicTargetVol(0.035);
        else
            this.audio.setBGMusicTargetVol(0.045);
        // Draw room
        drawRoom(this.g);
        ...
        let uy = uncleY;
        if (isCollapse || isIdle)
            uy = couchSeatY + this.collapseBounce;
        ...
        this.g.fillRect(sx - 12, sy + 22, 28, 5);
        // Legs
        this.g.fillStyle(pants);
        if (isWalking) {
            const leg = Math.sin(this.animTime * 0.01) * 4;
            this.g.fillRect(sx - 4, sy + 14, 7, 10 + leg);
            this.g.fillRect(sx + 3, sy + 14, 7, 10 - leg);
        }
        else {
            this.g.fillRect(sx - 4, sy + 14, 7, 12);
            this.g.fillRect(sx + 3, sy + 14, 7, 12);
        }
        // Body
        this.g.fillStyle(shirt);
        this.g.fillRect(sx - 14, sy - 4, 32, 20);
        this.g.fillStyle(shirtLight);
        this.g.fillRect(sx - 12, sy, 28, 12);
        this.g.fillStyle(0xffffff);
        this.g.fillRect(sx - 6, sy + 2, 5, 5);
        this.g.fillRect(sx + 6, sy + 2, 5, 5);
        // Head
        this.g.fillStyle(skin);
        this.g.fillRect(sx - 8, sy - 18, 24, 16);
        this.g.fillStyle(hairBrown);
        this.g.fillRect(sx - 8, sy - 22, 22, 6);
        this.g.fillRect(sx - 12, sy - 18, 6, 8);
        // Eyes
        this.g.fillStyle(0x000000);
        if (isIdle || (isCollapse && this.collapseT > 400)) {
            this.g.fillRect(sx - 2, sy - 10, 6, 1);
            this.g.fillRect(sx + 6, sy - 10, 6, 1);
        }
        else {
            this.g.fillRect(sx - 2, sy - 12, 4, 4);
            this.g.fillRect(sx + 6, sy - 12, 4, 4);
        }
        // Nose
        this.g.fillStyle(skinDark);
        this.g.fillRect(sx + 14, sy - 10, 4, 4);
        // Mouth
    ...
    ensureHiScoreText() {
        if (this.hiScoreText)
            this.hiScoreText.setText(`${t('hiScore')}: ${getTopScore()}`);
    }
    renderTitleHS() {
        ...
        if (this._hsTexts.length === 0) {
            const style = { fontFamily: '"Press Start 2P"', fontSize: '9px', color: C.uiText };
    ...
    renderTitleHow() {
        ...
        if (this._howTexts.length === 0) {
            const style = { fontFamily: '"Press Start 2P"', fontSize: '8px', color: C.uiText };
    ...
    handleKey(key) {
        ...
        if (key === 'ArrowUp') {
            this.titleCur = (this.titleCur + menuLen - 1) % menuLen;
            this.audio.playMenuNav();
        }
        if (key === 'ArrowDown') {
            this.titleCur = (this.titleCur + 1) % menuLen;
            this.audio.playMenuNav();
        }
        if (key === 'Enter') {
            ...
            else if (opt === 'highScores')
                this.titleHS = true;
            else if (opt === 'howToPlay')
                this.titleHow = true;
