...
export class GameAudio {
    ...
    set soundOn(val) { this._soundOn = val; }
    init() {
        if (!this.ctx) {
    ...
    playNote(freq, dur, type = 'square', vol = 0.1, ramp) {
        this.sfx(() => {
            const ctx = this.ctx;
            ...
            if (ramp)
                o.frequency.exponentialRampToValueAtTime(ramp, ctx.currentTime + dur);
    ...
    playFart(type) {
        if (type === 'quick')
            this.playNote(80, 0.12, 'sawtooth', 0.2, 30);
        else if (type === 'mega')
            this.playNote(55, 0.5, 'sawtooth', 0.3, 15);
        else
            this.playNote(120, 0.2, 'sine', 0.08, 50);
    }
    playSnore() { this.playNote(50, 0.4, 'sawtooth', 0.04, 65); }
    playHit() { this.playNote(300, 0.08, 'square', 0.1, 80); }
    playPoison() { this.playNote(180, 0.2, 'sine', 0.06, 60); }
    playWakeUp() { this.playNote(250, 0.15, 'sawtooth', 0.1, 500); }
    playWaveComplete() {
    ...
    playMenuNav() { this.playNote(880, 0.05, 'square', 0.06); }
    playMenuSelect() {
    ...
    // --- Background Music ---
    startBGMusic(targetVol = 0.045) {
        if (!this._soundOn || this.bgPlaying)
            return;
        this.init();
        this.bgPlaying = true;
        this.bgVolume = 0;
    ...
    setBGMusicTargetVol(vol) {
        this.bgTargetVol = vol;
    }
    updateBGMusic() {
        if (!this.bgPlaying && this.bgVolume <= 0)
            return;
        if (!this.ctx || !this._soundOn)
            return;
        // Volume ramp
