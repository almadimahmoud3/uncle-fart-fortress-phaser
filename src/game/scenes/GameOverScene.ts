...
export class GameOverScene extends Phaser.Scene {
  ...
  update(): void {
    ...
    if (this.texts.length === 0) {
      ...
      this.texts.push(this.add.text(320, 48, t('gameOver'), { ...style, fontSize: '22px', color: C.uiRed }).setOrigin(0.5, 0));
      this.texts.push(this.add.text(320, 82, t('uncleWoke'), { ...style, fontSize: '10px', color: C.uiTextLight }).setOrigin(0.5, 0));
      this.texts.push(this.add.text(320, 158, `${t('scoreLabel')}: ${this.score}`, { ...style, fontSize: '16px' }).setOrigin(0.5, 0));
      this.texts.push(this.add.text(320, 185, `${t('waveLabel')}: ${this.wave}  ${t('playerLabel')}: ${this.name}`, { ...style, fontSize: '9px', color: C.uiTextLight }).setOrigin(0.5, 0));
      this.texts.push(this.add.text(100, 265, t('topScores'), { ...style, fontSize: '9px', color: C.uiRed }));
      ...
      this.texts.push(this.add.text(200, 412, t('tryAgain'), { ...style, fontSize: '10px' }));
      this.texts.push(this.add.text(200, 436, t('mainMenu'), { ...style, fontSize: '10px' }));
    }
  }
