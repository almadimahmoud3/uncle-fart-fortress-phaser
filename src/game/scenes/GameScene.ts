...
interface Relative {
  ...
  poisoned: boolean; poisonTimer: number;
  wobble: number; hitFlash: number;
}

interface Fart {
  x: number; y: number; radius: number;
  createdAt: number; lifetime: number; type: string;
}

interface Particle {
  x: number; y: number; vx: number; vy: number;
  s: number; a: number; decay: number; col: string;
}

interface FloatText {
  x: number; y: number; t: string; col: string; sz: number;
  vy: number; a: number; decay: number;
...
export class GameScene extends Phaser.Scene {
  ...
  private snoreTmr = 0;
  private spawnTmr = 0;
  private relsInWave = 0;
  private relsSpawned = 0;
  ...
  private fCD = { quick: 0, mega: 0, silent: 0 };

  // UI text objects
  private sleepBarG!: Phaser.GameObjects.Graphics;
  private uiTexts: Phaser.GameObjects.Text[] = [];
  private waveAnnText?: Phaser.GameObjects.Text;

  // Emoji rendering
  private relEmojis: Map<Relative, Phaser.GameObjects.Text> = new Map();

  // Mobile touch controls
  private swipeStartX = 0;
  private swipeStartY = 0;
  private swipeStart_time = 0;
  private readonly SWIPE_THRESHOLD = 30;
  private readonly SWIPE_MAX_TIME = 400;
  private touchRipples: { x: number; y: number; r: number; alpha: number; col: string }[] = [];
  ...
  create(): void {
    ...
    this.uiTexts = [];
    this._fbtnTexts.forEach(tx => { if (tx) tx.destroy(); });
    this._fbtnTexts = [];
    Object.values(this._sideBtnTexts).forEach(tx => tx.destroy());
    this._sideBtnTexts = {};
    if (this.hintTxt) { this.hintTxt.destroy(); this.hintTxt = undefined; }
    if (this.waveAnnText) { this.waveAnnText.destroy(); this.waveAnnText = undefined; }
    ...
    this.input.keyboard!.on('keydown', (e: KeyboardEvent) => {
      if (e.key === 'Escape') { this.scene.launch('Pause'); this.scene.pause(); }
      if (e.key === '1') this.useFart('quick');
      if (e.key === '2') this.useFart('mega');
      if (e.key === '3') this.useFart('silent');
    });
    ...
    let pendingFartType: 'quick' | null = null;
    ...
    this.input.on('pointerdown', (pointer: Phaser.Input.Pointer) => {
      ...
      if (my >= 400 && my <= 480) {
        if (mx >= 20 && mx <= 190) { this.useFart('quick'); this.addRipple(mx, my, C.grass1); pendingTapActive = false; }
        else if (mx >= 200 && mx <= 370) { this.useFart('mega'); this.addRipple(mx, my, C.orange); pendingTapActive = false; }
        else if (mx >= 380 && mx <= 560) { this.useFart('silent'); this.addRipple(mx, my, C.fartPoison); pendingTapActive = false; }
        return;
      }
      ...
      if (mx >= 0 && mx <= 60) {
        if (my >= 140 && my < 200) { this.useFart('quick'); this.addRipple(mx, my, C.grass1); pendingTapActive = false; return; }
        if (my >= 220 && my < 280) { this.useFart('mega'); this.addRipple(mx, my, C.orange); pendingTapActive = false; return; }
        if (my >= 300 && my < 360) { this.useFart('silent'); this.addRipple(mx, my, C.fartPoison); pendingTapActive = false; return; }
      }
    ...
    this.input.on('pointerup', (pointer: Phaser.Input.Pointer) => {
      ...
      if (elapsed < this.SWIPE_MAX_TIME && dist > this.SWIPE_THRESHOLD) {
        ...
        if (Math.abs(dx) > Math.abs(dy)) {
          if (dx > 0) this.useFart('silent'); // swipe right = poison
          else this.useFart('mega');           // swipe left = mega
        } else {
          if (dy < 0) this.useFart('mega');    // swipe up = mega
          else this.useFart('quick');          // swipe down = tiny
        }
  ...
  update(_time: number, delta: number): void {
    ...
    // Update cooldowns
    for (const k of ['quick', 'mega', 'silent'] as const) {
      if (this.fCD[k] > 0) this.fCD[k] -= dt;
    }
    if (this.wakeCd > 0) this.wakeCd -= dt;
    if (this.fartAnimActive) { this.fartAnimTimer -= dt; if (this.fartAnimTimer <= 0) this.fartAnimActive = false; }
    if (this.waveAnnTmr > 0) this.waveAnnTmr -= dt;
    ...
    for (const r of [...this.relatives]) {
      ...
      if (dist > 18) {
        ...
        if (this.wakeCd <= 0) { this.audio.playWakeUp(); this.wakeCd = 500; }
        this.sleepLvl -= 20;
        if (this.sleepLvl <= 0) this.gameOver();
      }
    }
    ...
    for (const p of this.particles) {
      p.x += p.vx; p.y += p.vy; p.a -= p.decay; p.s = Math.max(1, p.s * 0.97);
    }
    this.particles = this.particles.filter(p => p.a > 0);

    // Update floats
    for (const f of this.floats) {
      f.y += f.vy; f.a -= f.decay;
      if (f.textObj) {
    ...
    for (const f of expiredFloats) {
      if (f.textObj) { f.textObj.destroy(); f.textObj = undefined; }
    }
    ...
    // Farts
    for (const f of this.farts) drawFart(this.g, f);
    ...
    this.relEmojis.forEach((t, r) => {
      if (!this.relatives.includes(r)) { t.destroy(); this.relEmojis.delete(r); }
    });

    // Particles
    for (const p of this.particles) {
      const col = Phaser.Display.Color.HexStringToColor(p.col).color;
      this.g.fillStyle(col, p.a); this.g.fillRect(p.x | 0, p.y | 0, p.s | 0, p.s | 0);
    }
    ...
    if (this.isMobile && this.wave === 1 && this.score === 0) {
      ...
      if (hintAlpha > 0) {
        ...
        if (!this.hintTxt) {
          const tStyle: Phaser.Types.GameObjects.Text.TextStyle = {
  ...
  private useFart(type: 'quick' | 'mega' | 'silent'): void {
    if (this.fCD[type] > 0) return;
    ...
    for (const r of this.relatives) {
      if (Math.hypot(r.x - (COUCH.x + COUCH.w / 2), r.y - COUCH.y) < fr) {
        ...
        if (type === 'silent' && !r.poisoned) { r.poisoned = true; r.poisonTimer = 3000; this.audio.playPoison(); }
        if (r.hp <= 0) {
          const pts = r.type.pts * this.wave;
          this.score += pts;
  ...
  private addRipple(x: number, y: number, col: string): void {
    this.touchRipples.push({ x, y, r: 8, alpha: 1, col });
  }

  private addFloat(x: number, y: number, t: string, col: string, sz: number): void {
  ...
  private drawUI(): void {
    ...
    this.g.fillStyle(black); this.g.fillRect(16, 28, 136, 8);
    ...
    this.g.fillStyle(sleepCol); this.g.fillRect(16, 28, (136 * this.sleepLvl / 100) | 0, 8);
  ...
  private drawFBtn(x: number, y: number, key: string, label: string, cd: number, maxCd: number, col: string): void {
    const w = 130, h = 50;
    const ready = cd <= 0;
    const prog = ready ? 1 : 1 - cd / maxCd;
    drawPokeBox(this.g, x, y, w, h);

    this.g.fillStyle(0x000000); this.g.fillRect(x + 8, y + 36, w - 16, 8);
    const colHex = Phaser.Display.Color.HexStringToColor(col).color;
    this.g.fillStyle(colHex); this.g.fillRect(x + 8, y + 36, ((w - 16) * prog) | 0, 8);
    this.g.fillStyle(0xffffff); this.g.fillRect(x + 8, y + 36, ((w - 16) * prog) | 0, 2);
    if (ready && Math.floor(Date.now() / 400) % 2 === 0) {
      this.g.fillStyle(0xffffff); this.g.fillRect(x + 8, y + 36, w - 16, 2);
    }
    ...
    if (!this._fbtnTexts[existingIdx]) {
      const tStyle: Phaser.Types.GameObjects.Text.TextStyle = { fontFamily: '"Press Start 2P"', fontSize: '8px', color: C.uiText };
  ...
  private _fbtnTexts: Phaser.GameObjects.Text[] = [];

  private drawPauseBtn(x: number, y: number): void {
    const size = 28;
    const uiRed = Phaser.Display.Color.HexStringToColor(C.uiRed).color;
  ...
  private drawSideBtn(x: number, y: number, label: string, cd: number, maxCd: number, col: string): void {
    const w = 50, h = 60;
    const ready = cd <= 0;
    const prog = ready ? 1 : 1 - cd / maxCd;
    ...
    if (!this._sideBtnTexts[sideKey]) {
      const tStyle: Phaser.Types.GameObjects.Text.TextStyle = { fontFamily: '"Press Start 2P"', fontSize: '6px', color: C.uiText };
      this._sideBtnTexts[sideKey] = this.add.text(x + w / 2, y + h / 2 - 6, label, tStyle).setOrigin(0.5, 0);
    }
  }
  private _sideBtnTexts: Record<string, Phaser.GameObjects.Text> = {};
