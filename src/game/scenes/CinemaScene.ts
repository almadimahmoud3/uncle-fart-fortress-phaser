...
export class CinemaScene extends Phaser.Scene {
  ...
  private timer = 0;
  private sceneIdx = 0;
  private sceneDur = 6500;
  private fadeDur = 800;

  // Pre-created text objects (reused every frame)
  private skipTxt!: Phaser.GameObjects.Text;
  private captionTxt!: Phaser.GameObjects.Text;
  private caption2Txt!: Phaser.GameObjects.Text;
  private bigTxt!: Phaser.GameObjects.Text;
  ...
  update(_time: number, delta: number): void {
    ...
    if (this.sceneIdx === 0) this.renderScene0(t, fadeOut);
    else if (this.sceneIdx === 1) this.renderScene1(t, fadeOut);
    else if (this.sceneIdx === 2) this.renderScene2(t, fadeOut);
    else if (this.sceneIdx === 3) this.renderScene3(t);
  ...
  private setCaption(line1: string, line2: string, alpha: number): void {
    this.captionTxt.setText(line1).setAlpha(alpha);
    this.caption2Txt.setText(line2).setAlpha(alpha);
  }

  private renderScene0(t: number, fadeOut: number): void {
    ...
    this.g.fillStyle(sky); this.g.fillRect(0, 0, 640, 260);
    this.g.fillStyle(skyLight); this.g.fillRect(0, 200, 640, 60);
    ...
    this.g.fillStyle(grass2); this.g.fillRect(0, 260, 640, 220);
    this.g.fillStyle(grass1); for (let i = 0; i < 20; i++) this.g.fillRect(i * 34, 258, 20, 6);
    this.g.fillStyle(gray); this.g.fillRect(0, 330, 640, 60);
    this.g.fillStyle(lightGray); for (let i = 0; i < 16; i++) this.g.fillRect(20 + i * 40, 356, 20, 4);
    this.g.fillStyle(0xe8d5b7); this.g.fillRect(380, 150, 200, 110);
    this.g.fillStyle(couchDark); this.g.fillRect(370, 130, 220, 24);
    this.g.fillStyle(couchBody); this.g.fillRect(375, 132, 210, 18);
    this.g.fillStyle(couchDark); this.g.fillRect(460, 190, 40, 70);
    this.g.fillStyle(couchBody); this.g.fillRect(464, 194, 32, 62);
    this.g.fillStyle(yellow); this.g.fillRect(490, 228, 4, 4);
    this.g.fillStyle(windowFrame); this.g.fillRect(400, 175, 35, 30);
    this.g.fillStyle(windowGlass); this.g.fillRect(404, 179, 27, 22);
    this.g.fillStyle(trunk); this.g.fillRect(280, 190, 12, 70);
    this.g.fillStyle(tree2); this.g.fillRect(260, 160, 52, 40);
    this.g.fillStyle(tree1); this.g.fillRect(268, 148, 36, 20);
    ...
    this.g.fillStyle(shirt); this.g.fillRect(ux - 14, uy - 4, 32, 20);
    this.g.fillStyle(shirtLight); this.g.fillRect(ux - 12, uy, 28, 12);
    ...
    this.g.fillStyle(skinDark); this.g.fillRect(ux + 14, uy - 10, 4, 4);
    this.g.fillStyle(0xc62828); this.g.fillRect(ux + 14, uy - 4, 5, 2);
  ...
  private renderScene1(t: number, alpha: number): void {
    ...
    this.g.fillStyle(wallTop); this.g.fillRect(0, 0, 640, 260);
    this.g.fillStyle(wallAccent);
    for (let y = 8; y < 260; y += 20) for (let x = (y % 40 === 8 ? 0 : 16); x < 640; x += 32) this.g.fillRect(x, y, 28, 16);
    this.g.fillStyle(wallBot); this.g.fillRect(0, 220, 640, 40);
    this.g.fillStyle(wallAccent); this.g.fillRect(0, 220, 640, 3);
    this.g.fillStyle(floor); this.g.fillRect(0, 260, 640, 220);
    for (let y = 260; y < 480; y += 32) for (let x = 0; x < 640; x += 32) {
      this.g.fillStyle((x + y) % 64 === 0 ? floorTile : floor);
      this.g.fillRect(x, y, 32, 32);
    }
    this.g.fillStyle(couchDark); this.g.fillRect(480, 60, 60, 160);
    this.g.fillStyle(couchBody); this.g.fillRect(484, 64, 52, 152);
    this.g.fillStyle(yellow); this.g.fillRect(526, 140, 4, 4);
    this.g.fillStyle(couchDark); this.g.fillRect(540, 180, 80, 30);
    this.g.fillStyle(couchBody); this.g.fillRect(544, 184, 72, 22);
    ...
    if (walkIn < 0.95) { const leg = Math.sin(t * 0.008) * 5; this.g.fillRect(ux - 4, uy + 14, 7, 12 + leg); this.g.fillRect(ux + 3, uy + 14, 7, 12 - leg); }
    else { this.g.fillRect(ux - 4, uy + 14, 7, 14); this.g.fillRect(ux + 3, uy + 14, 7, 14); }
    ...
    if (rp < 0.3) {
      ...
      this.g.fillStyle(shirt); this.g.fillRect(ux - 14, uy - 4 - liftY * 30, 32, 20);
      this.g.fillStyle(skin); this.g.fillRect(ux - 14, uy + 4, 32, 14);
    } else {
      const flyT = (rp - 0.6) / 0.4;
      const flyX = ux + flyT * 120, flyY = uy - 40 - Math.sin(flyT * Math.PI) * 50;
      this.g.fillStyle(shirt); this.g.fillRect((flyX - 14) | 0, (flyY - 8) | 0, 28, 16);
    ...
    this.g.fillStyle(skin); this.g.fillRect(ux - 8, uy - 18, 24, 16);
    this.g.fillStyle(hairBrown); this.g.fillRect(ux - 8, uy - 22, 22, 6);
    this.g.fillStyle(0x000000); this.g.fillRect(ux - 2, uy - 10, 4, 3); this.g.fillRect(ux + 6, uy - 10, 4, 3);
    this.g.fillStyle(skinDark); this.g.fillRect(ux + 14, uy - 10, 4, 4);
    this.g.fillStyle(0xc62828); this.g.fillRect(ux + 14, uy - 4, 5, 2);
  ...
  private renderScene2(t: number, alpha: number): void {
    drawRoom(this.g);
    ...
    this.g.fillStyle(couchDark); this.g.fillRect(couchX, couchY - 18, 140, 22);
    this.g.fillStyle(couchBody); this.g.fillRect(couchX + 2, couchY - 16, 136, 18);
    this.g.fillStyle(couchDark); this.g.fillRect(couchX - 10, couchY - 12, 14, 57);
    this.g.fillRect(couchX + 136, couchY - 12, 14, 57);
    this.g.fillStyle(couchBody); this.g.fillRect(couchX, couchY, 140, 45);
    this.g.fillStyle(couchCushion); this.g.fillRect(couchX + 6, couchY + 4, 128, 31);
    ...
    if (walkToCouch < 1) {
      ux = (80 + walkToCouch * 240) | 0; uy = 310;
      ...
      uy = couchY - 8 + bounce;
      this.g.fillStyle(skin); this.g.fillRect(ux - 28, uy + 4, 56, 20);
      this.g.fillStyle(skinShadow); this.g.fillRect(ux - 24, uy + 10, 48, 12);
      this.g.fillStyle(pants); this.g.fillRect(ux - 20, uy + 20, 40, 14);
      this.g.fillStyle(skin); this.g.fillRect(ux - 18, uy - 12, 36, 20);
      this.g.fillStyle(hairBrown); this.g.fillRect(ux - 18, uy - 16, 32, 8);
      const eyeClose = Math.min(1, sinkProg * 2.5);
      this.g.fillStyle(0x000000);
      if (eyeClose < 0.6) { this.g.fillRect(ux - 10, uy - 4, 6, 3); this.g.fillRect(ux + 6, uy - 4, 6, 3); }
      else { this.g.fillRect(ux - 10, uy - 4, 6, 1); this.g.fillRect(ux + 6, uy - 4, 6, 1); }
      this.g.fillStyle(0xc62828); this.g.fillRect(ux + 14, uy - 2, 5, 3);
    }
  ...
  private renderScene3(t: number): void {
    drawRoom(this.g);
    const couchDark = Phaser.Display.Color.HexStringToColor(C.couchDark).color;
    const couchBody = Phaser.Display.Color.HexStringToColor(C.couchBody).color;
    const couchCushion = Phaser.Display.Color.HexStringToColor(C.couchCushion).color;
    this.g.fillStyle(couchDark); this.g.fillRect(250, 262, 140, 22);
    this.g.fillStyle(couchBody); this.g.fillRect(252, 264, 136, 18);
    this.g.fillStyle(couchDark); this.g.fillRect(240, 268, 14, 57); this.g.fillRect(386, 268, 14, 57);
    this.g.fillStyle(couchBody); this.g.fillRect(250, 280, 140, 45);
    this.g.fillStyle(couchCushion); this.g.fillRect(256, 284, 128, 31);
    ...
    this.g.fillStyle(skin); this.g.fillRect(ux - 18, uy - 12, 36, 20);
    this.g.fillStyle(hairBrown); this.g.fillRect(ux - 18, uy - 16, 32, 8);
    this.g.fillStyle(0x000000); this.g.fillRect(ux - 10, uy - 4, 6, 1); this.g.fillRect(ux + 6, uy - 4, 6, 1);
    const mo = Math.sin(t * 0.004) * 2 + 2;
    this.g.fillStyle(0xc62828); this.g.fillRect(ux + 14, uy - 2, 5, (mo | 0) + 2);
    ...
    for (let i = 0; i < numZ; i++) {
      ...
      this.g.fillRect(zx, zy, s * 3, s); this.g.fillRect(zx + s * 2, zy, s, s);
      this.g.fillRect(zx + s, zy + s, s, s); this.g.fillRect(zx, zy + s * 2, s, s);
    ...
    if (zProg > 0.4) {
      ...
      this.g.fillStyle(bg, cp * 0.9); this.g.fillCircle(ux + 25, uy - 15, cp * 500);
    }
    ...
    if (t > 4000) {
      ...
      const bg = Phaser.Display.Color.HexStringToColor(C.uiBg).color;
      this.g.fillStyle(bg, fp); this.g.fillRect(0, 0, 640, 480);
    }
    this.setCaption('', '', 0);
  }
}
