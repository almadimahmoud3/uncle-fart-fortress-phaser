...
export class PauseScene extends Phaser.Scene {
  ...
  private cur = 0;
  private opts = ['RESUME', 'SOUND: ON', 'QUIT TO MENU'];
  ...
  create(): void {
    ...
    this.input.on('pointerdown', (pointer: Phaser.Input.Pointer) => {
      ...
      if (mx > 160 && mx < 480) {
        if (my > 180 && my < 220) this.doAction(0);
        if (my > 220 && my < 260) this.doAction(1);
        if (my > 260 && my < 300) this.doAction(2);
      }
    });
  }
  ...
  update(): void {
    this.g.clear();
    // Dim background
    this.g.fillStyle(0x000000, 0.6); this.g.fillRect(0, 0, GAME_W, GAME_H);
  ...
  private handleKey(key: string): void {
    if (key === 'Escape') { this.scene.resume('Game'); this.scene.stop(); }
    if (key === 'ArrowUp') { this.cur = (this.cur + 2) % 3; this.audio.playMenuNav(); }
    if (key === 'ArrowDown') { this.cur = (this.cur + 1) % 3; this.audio.playMenuNav(); }
    if (key === 'Enter') { this.doAction(this.cur); }
  }

  private doAction(idx: number): void {
    this.audio.playMenuSelect();
    if (idx === 0) { this.scene.resume('Game'); this.scene.stop(); }
    if (idx === 1) {
      this.soundOn = !this.soundOn;
      this.registry.set('soundOn', this.soundOn);
      this.opts[1] = `SOUND: ${this.soundOn ? 'ON' : 'OFF'}`;
      if (this.texts[2]) this.texts[2].setText(this.opts[1]);
    }
    if (idx === 2) {
