...
export function drawPokeBox(g: Phaser.GameObjects.Graphics, x: number, y: number, w: number, h: number): void {
...
export function drawRoom(g: Phaser.GameObjects.Graphics): void {
  ...
  // Walls
  g.fillStyle(wallTop); g.fillRect(0, 0, 640, 180);
  ...
  g.fillStyle(wallBot); g.fillRect(0, 140, 640, 40);
  g.fillStyle(wallAccent); g.fillRect(0, 140, 640, 3);

  // Floor
  g.fillStyle(floor); g.fillRect(0, 180, 640, 300);
  for (let y = 180; y < 480; y += 32) {
    for (let x = 0; x < 640; x += 32) {
      g.fillStyle((x + y) % 64 === 0 ? floorTile : floor);
      g.fillRect(x, y, 32, 32);
      g.fillStyle(floorLine); g.fillRect(x, y, 32, 1); g.fillRect(x, y, 1, 32);
    }
  }

  // Window
  g.fillStyle(windowFrame); g.fillRect(40, 25, 90, 85);
  g.fillStyle(skyLight); g.fillRect(44, 29, 82, 77);
  g.fillStyle(sky);
  g.fillRect(48, 33, 36, 33); g.fillRect(88, 33, 36, 33);
  g.fillRect(48, 70, 36, 32); g.fillRect(88, 70, 36, 32);
  g.fillStyle(windowFrame); g.fillRect(84, 29, 4, 77); g.fillRect(44, 65, 82, 4);
  g.fillStyle(white);
  g.fillRect(52, 37, 12, 6); g.fillRect(56, 34, 8, 4);
  g.fillRect(92, 42, 10, 5); g.fillRect(95, 39, 6, 4);
  g.fillStyle(curtain); g.fillRect(28, 18, 16, 98); g.fillRect(126, 18, 16, 98);
  g.fillStyle(curtainLight); g.fillRect(32, 18, 4, 98); g.fillRect(130, 18, 4, 98);

  // Picture frame
  const border = Phaser.Display.Color.HexStringToColor(C.uiBorder).color;
  g.fillStyle(border); g.fillRect(500, 40, 60, 50);
  g.fillStyle(grass1); g.fillRect(504, 44, 52, 42);
  g.fillStyle(grass2); g.fillRect(504, 60, 52, 26);
  g.fillStyle(trunk); g.fillRect(528, 52, 6, 20);
  g.fillStyle(tree1); g.fillRect(520, 42, 22, 14);
}

/** Draw the couch */
export function drawCouch(g: Phaser.GameObjects.Graphics, cx: number, cy: number, cw: number, ch: number): void {
  ...
  // Shadow
  g.fillStyle(0x000000, 0.15); g.fillRect(cx + 3, cy + 3, cw, ch);
  // Back
  g.fillStyle(dark); g.fillRect(cx, cy - 18, cw, 22);
  g.fillStyle(body); g.fillRect(cx + 2, cy - 16, cw - 4, 18);
  // Arms
  g.fillStyle(dark);
  ...
  // Body
  g.fillStyle(body); g.fillRect(cx, cy, cw, ch);
  g.fillStyle(cushion); g.fillRect(cx + 6, cy + 4, cw - 12, ch - 14);
  // Cushion lines
  g.fillStyle(body);
  for (let i = cx + 14; i < cx + cw - 10; i += 18) g.fillRect(i, cy + 8, 2, ch - 22);
}

/** Draw the uncle sleeping sprite */
export function drawUncleSprite(
  g: Phaser.GameObjects.Graphics,
  x: number, y: number, snoring: boolean, shaking: boolean
): void {
  ...
  // Body (pajamas)
  g.fillStyle(shirt); g.fillRect(sx - 28, sy + 8, 56, 24);
  g.fillStyle(shirtLight); g.fillRect(sx - 24, sy + 12, 48, 16);
  g.fillStyle(white);
  g.fillRect(sx - 16, sy + 14, 6, 6); g.fillRect(sx + 8, sy + 14, 6, 6);

  // Head
  g.fillStyle(skin); g.fillRect(sx - 20, sy - 14, 36, 28);
  g.fillStyle(skinShadow); g.fillRect(sx - 18, sy + 8, 32, 6);

  // Hair
  g.fillStyle(hairBrown); g.fillRect(sx - 20, sy - 18, 32, 8); g.fillRect(sx - 24, sy - 14, 8, 10);

  // Closed eyes
  g.fillStyle(uiText); g.fillRect(sx - 12, sy - 4, 6, 3); g.fillRect(sx + 4, sy - 4, 6, 3);

  // Nose
  g.fillStyle(skinDark); g.fillRect(sx + 14, sy - 2, 5, 5);

  // Mouth
  const mo = snoring ? Math.sin(Date.now() / 200) * 2 + 3 : 2;
  const mouth = Phaser.Display.Color.HexStringToColor(C.mouth).color;
  g.fillStyle(mouth); g.fillRect(sx + 16, sy + 6, 6, (mo | 0) + 2);
}

/** Draw a relative (enemy) sprite */
export function drawRelativeSprite(
  g: Phaser.GameObjects.Graphics,
  rel: { x: number; y: number; hp: number; maxHp: number; type: { col: string; emoji: string }; poisoned: boolean; hitFlash: number }
): void {
  const x = rel.x | 0, y = rel.y | 0;
  const sz = 20;

  // Shadow
  g.fillStyle(0x000000, 0.2); g.fillRect(x - sz / 2, y + sz, sz, 4);
  // Body
  const col = Phaser.Display.Color.HexStringToColor(rel.type.col).color;
  g.fillStyle(col); g.fillRect(x - sz / 2, y - sz / 2, sz, sz);
  // Face
  const skin = Phaser.Display.Color.HexStringToColor(C.skin).color;
  g.fillStyle(skin); g.fillRect(x - sz / 2 + 3, y - sz / 2 + 3, sz - 6, sz - 6);
  // Eyes
  g.fillStyle(0x000000);
  g.fillRect(x - 5, y - 3, 3, 3); g.fillRect(x + 2, y - 3, 3, 3);
  // Eyebrows
  g.fillRect(x - 7, y - 7, 5, 2); g.fillRect(x + 2, y - 7, 5, 2);
  // Mouth
  g.fillRect(x - 3, y + 3, 6, 2);
  // HP bar
  if (rel.maxHp > 1) {
    g.fillStyle(0x000000); g.fillRect(x - 12, y - sz / 2 - 6, 24, 4);
    ...
    g.fillStyle(hpCol); g.fillRect(x - 12, y - sz / 2 - 6, (24 * rel.hp / rel.maxHp) | 0, 4);
  }
  // Poisoned overlay
  if (rel.poisoned) {
    const poison = Phaser.Display.Color.HexStringToColor(C.fartPoison).color;
    g.fillStyle(poison, 0.25); g.fillRect(x - sz / 2 - 2, y - sz / 2 - 2, sz + 4, sz + 4);
  }
  // Hit flash
  if (rel.hitFlash > 0) {
    g.fillStyle(0xffffff, rel.hitFlash / 8); g.fillRect(x - sz / 2, y - sz / 2, sz, sz);
  }
}

/** Draw a fart cloud */
export function drawFart(
  g: Phaser.GameObjects.Graphics,
  f: { x: number; y: number; radius: number; createdAt: number; lifetime: number; type: string }
): void {
...
export function drawParticle(g: Phaser.GameObjects.Graphics, p: { x: number; y: number; s: number; a: number; col: string }): void {
